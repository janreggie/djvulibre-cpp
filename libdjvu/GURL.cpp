...
// Defined as isArgumentInit in go-djvulibre
static bool
...
void GURL::convert_slashes(void)
{
...
void GURL::beautify_path(void)
{
    url = beautify_path(get_string());
}

void GURL::init(const bool nothrow)
{
    GCriticalSectionLock lock(&class_lock); // We don't need to lock in the go imp'l b/c this is a private method
    validurl = true;
...
void GURL::add_djvu_cgi_argument(const GUTF8String &name, const char *value)
{
    if (!validurl)
        init();
    GCriticalSectionLock lock1(&class_lock);
...
GURL::decode_reserved(const GUTF8String &gurl)
{
    ...
    for (const char *ptr = url; *ptr; ++ptr, ++r)
    {
        ...
        {
            int c1, c2;
            if (((c1 = hexval(ptr[1])) >= 0) && ((c2 = hexval(ptr[2])) >= 0))
            {
                r[0] = (c1 << 4) | c2;
                ptr += 2;
            }
...
url_from_UTF8filename(const GUTF8String &gfilename)
{
    ...
    if (filename && (unsigned char)filename[0] == (unsigned char)0xEF && (unsigned char)filename[1] == (unsigned char)0xBB && (unsigned char)filename[2] == (unsigned char)0xBF)
    {
        filename += 3;
    }
    ...
    if (!is_empty())
    {
        ...
        if (!GStringRep::cmp(localhostspec1, url_ptr, sizeof(localhostspec1) - 1))
            ...
            url_ptr += sizeof(localhostspec2) - 1;
                 ...
                 && (url_ptr[1] == slash) && isalpha(url_ptr[2]) && (url_ptr[3] == colon || url_ptr[3] == '|') && (url_ptr[4] == slash))
            url_ptr += 2;
        else if ((strlen(url_ptr)) > 2 // "file:/<path>"
                 && (url_ptr[0] == slash) && (url_ptr[1] != slash))
            url_ptr++;
#endif
        ...
        {
            if ((url_ptr[0] >= 'a' && url_ptr[0] <= 'z') || (url_ptr[0] >= 'A' && url_ptr[0] <= 'Z'))
            {
                GUTF8String drive;
...
int GURL::cleardir(const int timeout) const
{
    int retval = (-1);
    if (is_dir())
    {
    ...
    if (fname)
    {
        ...
        while (*fname)
        {
            ...
            if (fname[0] == dot)
            {
                ...
                else if ((fname[1] == dot) && (fname[2] == slash || fname[2] == backslash || !fname[2]))
                {
                    fname += 2;
