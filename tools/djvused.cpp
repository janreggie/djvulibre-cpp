...
static GUTF8String ToNative(GUTF8String s) {
  if (utf8)
    return s;
...
int ParsingByteStream::eof() // aka. feof
{
  if (bufpos < bufend)
    return false;
  if (goteof)
...
long int ParsingByteStream::tell() const {
  G_THROW("Cannot tell() a ParsingByteStream");
  return 0;
}

int ParsingByteStream::getbom(int c) {
  int i = 0;
  while (c == utf8bom[i++]) {
    if (i >= 3)
      i = 0;
...
inline int ParsingByteStream::get() // like getc() skipping bom.
{
  int c = EOF;
  if (bufpos < bufend || !eof())
    c = buffer[bufpos++];
...
int ParsingByteStream::unget(int c) // like ungetc()
{
  if (bufpos > 0 && c != EOF)
...
const char *ParsingByteStream::get_error_context(int c) {
...
GUTF8String ParsingByteStream::get_token(bool skipseparator, bool compat) {
  GUTF8String str;
...
static void verror(const char *fmt, ...)
#ifdef __GNUC__
    __attribute__((format(printf, 1, 2)));
static void verror(const char *fmt, ...)
#endif
...
static void get_data_from_file(const char *cmd, ParsingByteStream &pbs,
                               ByteStream &out) {
...
static bool char_unquoted(unsigned char c, bool eightbit) {
  if (eightbit && c >= 0x80)
    return true;
  if (c == 0x7f || c == '\"' || c == '\\')
    return false;
...
static void print_c_string(const char *data, int length, ByteStream &out,
                           bool eightbit) {
  out.write("\"", 1);
  while (*data && length > 0) {
    int span = 0;
...
void command_ls(ParsingByteStream &) {
  int pagenum = 0;
...
void command_n(ParsingByteStream &) {
  int pagenum = 0;
  GPList<DjVmDir::File> lst = g().doc->get_djvm_dir()->get_files_list();
  { // extra nesting for windows
    for (GPosition p = lst; p; ++p) {
...
void command_dump(ParsingByteStream &) {
  GP<DataPool> pool;
...
static void print_size(const GP<DjVuFile> &file) {
...
void command_size(ParsingByteStream &) {
...
static void select_add(GP<DjVmDir::File> frec) {
...
void command_select(ParsingByteStream &pbs) {
...
void command_select_shared_ant(ParsingByteStream &) {
...
void command_create_shared_ant(ParsingByteStream &) {
  GP<DjVmDir::File> frec = g().doc->get_djvm_dir()->get_shared_anno_file();
  if (!frec) {
...
void command_showsel(ParsingByteStream &) {
  int pagenum = 0;
  GPList<DjVmDir::File> &lst = g().selected;
  { // extra nesting for windows
    for (GPosition p = lst; p; ++p) {
...
void command_set_page_title(ParsingByteStream &pbs) {
  if (!g().file)
...
static bool filter_ant(GP<ByteStream> in, GP<ByteStream> out, int flags) {
  int c;
  int plevel = 0;
  bool copy = true;
  bool unchanged = true;
...
static bool print_ant(GP<IFFByteStream> iff, GP<ByteStream> out,
                      int flags = CHKCOMPAT) {
  GUTF8String chkid;
  bool changed = false;
  if (utf8)
...
void command_print_ant(ParsingByteStream &) {
  if (!g().file)
...
void command_print_merged_ant(ParsingByteStream &) {
  if (!g().file)
    verror("you must first select a single page");
  GP<ByteStream> out = ByteStream::create("w");
...
static void modify_ant(const GP<DjVuFile> &f, const char *newchunkid,
...
void file_remove_ant(const GP<DjVuFile> &f, const char *id) {
  if (!f)
    return;
  modify_ant(f, 0, 0);
...
void command_remove_ant(ParsingByteStream &) {
  GPList<DjVmDir::File> &lst = g().selected;
  { // extra nesting for windows
    for (GPosition p = lst; p; ++p) {
...
void command_set_ant(ParsingByteStream &pbs) {
  if (!g().file)
    verror("must select a single page first");
...
static void print_meta(IFFByteStream &iff, ByteStream &out) {
  GUTF8String chkid;
  while (iff.get_chunk(chkid)) {
    bool ok = false;
...
void command_print_meta(ParsingByteStream &) {
  if (!g().file) {
    GP<DjVmDir::File> frec = g().doc->get_djvm_dir()->get_shared_anno_file();
    if (frec) {
...
static bool modify_meta(const GP<DjVuFile> &f,
...
void file_remove_meta(const GP<DjVuFile> &f, const char *id) {
  if (modify_meta(f, 0))
    vprint("remove_meta: modified \"%s\"", id);
}

void command_remove_meta(ParsingByteStream &) {
  GPList<DjVmDir::File> &lst = g().selected;
  { // extra nesting for windows
    for (GPosition p = lst; p; ++p) {
      GUTF8String id = lst[p]->get_load_name();
...
void command_set_meta(ParsingByteStream &pbs) {
  // get metadata
  ...
  if (g().file && modify_meta(g().file, &metadata))
    vprint("set-meta: modified \"%s\"", (const char *)ToNative(g().fileid));
}

static void print_xmp(IFFByteStream &iff, ByteStream &out) {
  GUTF8String chkid;
  while (iff.get_chunk(chkid)) {
    bool ok = false;
    GP<DjVuANT> ant = DjVuANT::create();
...
void command_print_xmp(ParsingByteStream &) {
  if (!g().file) {
    GP<DjVmDir::File> frec = g().doc->get_djvm_dir()->get_shared_anno_file();
    if (frec) {
...
static bool modify_xmp(const GP<DjVuFile> &f, GUTF8String *newxmp) {
  bool changed = false;
  GP<ByteStream> newant = ByteStream::create();
...
void file_remove_xmp(const GP<DjVuFile> &f, const char *id) {
  if (modify_xmp(f, 0))
    vprint("remove_xmp: modified \"%s\"", id);
}

void command_remove_xmp(ParsingByteStream &) {
  GPList<DjVmDir::File> &lst = g().selected;
  { // extra nesting for windows
    for (GPosition p = lst; p; ++p) {
      GUTF8String id = lst[p]->get_load_name();
...
void command_set_xmp(ParsingByteStream &pbs) {
  // get xmpmetadata
  GP<ByteStream> metastream = ByteStream::create();
  get_data_from_file("set-meta", pbs, *metastream);
  metastream->seek(0);
  ...
  if (g().file && modify_xmp(g().file, &xmpmetadata))
    vprint("set-xmp: modified \"%s\"", (const char *)ToNative(g().fileid));
}
...
GP<DjVuTXT> get_text(const GP<DjVuFile> &file) {
  GUTF8String chkid;
...
void print_txt_sub(const GP<DjVuTXT> &txt, DjVuTXT::Zone &zone,
  ...
  if (zone_names()[zinfo].name)
    message.format("(%s %d %d %d %d", zone_names()[zinfo].name, zone.rect.xmin,
                   zone.rect.ymin, zone.rect.xmax, zone.rect.ymax);
...
void print_txt(const GP<DjVuTXT> &txt, const GP<ByteStream> &out) {
  if (txt)
    print_txt_sub(txt, txt->page_zone, out, 0);
}

void command_print_txt(ParsingByteStream &) {
...
void command_print_pure_txt(ParsingByteStream &) {
  const GP<ByteStream> out = ByteStream::create("w");
  GP<DjVuTXT> txt;
  GPList<DjVmDir::File> &lst = g().selected;
  { // extra nesting for windows
...
static void modify_txt(const GP<DjVuFile> &f, const char *newchunkid,
                       const GP<ByteStream> newchunk) {
...
void file_remove_txt(const GP<DjVuFile> &f, const char *id) {
  if (!f)
    return;
  modify_txt(f, 0, 0);
...
void command_remove_txt(ParsingByteStream &) {
  GPList<DjVmDir::File> &lst = g().selected;
  { // extra nesting for windows
    for (GPosition p = lst; p; ++p) {
      GUTF8String id = lst[p]->get_load_name();
...
void construct_djvutxt_sub(ParsingByteStream &pbs, const GP<DjVuTXT> &txt,
                           DjVuTXT::Zone &zone, int mintype, bool exact) {
  int c;
  GUTF8String token;
  // Get zone type
  ...
  if (!str || !str.is_int())
  nerror:
    verror("Syntax error in txt data: number expected,\n\tnear '%s%s'",
...
GP<DjVuTXT> construct_djvutxt(ParsingByteStream &pbs) {
  GP<DjVuTXT> txt(DjVuTXT::create());
  int c = pbs.get_spaces(true);
  if (c == EOF)
    return 0;
...
void command_set_txt(ParsingByteStream &pbs) {
  if (!g().file)
    verror("must select a single page first");
...
void output(const GP<DjVuFile> &f, const GP<ByteStream> &out, int flag,
            const char *id = 0) {
  if (f) {
    const GP<ByteStream> ant(ByteStream::create());
...
void command_output_ant(ParsingByteStream &) {
  const GP<ByteStream> out = ByteStream::create("w");
  if (g().file) {
    output(g().file, out, 1);
  } else {
...
void command_output_txt(ParsingByteStream &) {
  const GP<ByteStream> out = ByteStream::create("w");
  if (g().file) {
    output(g().file, out, 2);
  } else {
...
void command_output_all(ParsingByteStream &) {
  const GP<ByteStream> out = ByteStream::create("w");
  if (g().file) {
    output(g().file, out, 3);
  } else {
...
void print_outline_sub(const GP<DjVmNav> &nav, int &pos, int count,
                       const GP<ByteStream> &out, int indent) {
  GUTF8String str;
...
void command_print_outline(ParsingByteStream &pbs) {
...
void construct_outline_sub(ParsingByteStream &pbs, GP<DjVmNav> nav,
                           int &count) {
  int c;
...
GP<DjVmNav> construct_outline(ParsingByteStream &pbs) {
...
void command_set_outline(ParsingByteStream &pbs) {
...
static bool callback_thumbnails(int page_num, void *) {
  vprint("set-thumbnails: processing page %d", page_num + 1);
  return false;
}

void command_set_thumbnails(ParsingByteStream &pbs) {
...
void command_remove_thumbnails(ParsingByteStream &) {
  g().doc->remove_thumbnails();
  modified = true;
}

void command_save_page(ParsingByteStream &pbs) {
  GUTF8String fname = pbs.get_token();
  if (!fname)
  ...
  const GP<ByteStream> out(
      ByteStream::create(GURL::Filename::UTF8(fname), "wb"));
  out->writall("AT&T", 4);
  out->copy(*bs);
...
void command_save_page_with(ParsingByteStream &pbs) {
  GUTF8String fname = pbs.get_token();
  if (!fname)
    verror("empty filename");
  if (!g().file)
...
void command_save_bundled(ParsingByteStream &pbs) {
  GUTF8String fname = pbs.get_token();
  if (!fname)
    verror("empty filename");
  if (nosave)
...
void command_save_indirect(ParsingByteStream &pbs) {
  GUTF8String fname = pbs.get_token();
  if (!fname)
    verror("empty filename");
  if (nosave)
...
void command_save(ParsingByteStream &) { command_save(); }
...
void command_help(void) {
  fprintf(
      ...
      "Comment lines start with '#'.  Commands usually operate on pages and "
      "files\n"
      "specified by the \"select\" command.  All pages and files are initially "
      "selected.\n"
      "A single page must be selected before executing commands marked with a "
      "period.\n"
      ...
      "   size                   -- prints page width and height in html "
      "friendly way\n"
      "   select                 -- selects the entire document\n"
      "   select <id>            -- selects a single page/file by name or page "
      "number\n"
      "   select-shared-ant      -- selects the shared annotations file\n"
      "   create-shared-ant      -- creates and select the shared annotations "
      "file\n"
      "   showsel                -- displays currently selected pages/files\n"
      " . print-ant              -- prints annotations\n"
      " . print-merged-ant       -- prints annotations including the shared "
      "annotations\n"
      " . print-meta             -- prints file metadatas (a subset of the "
      "annotations\n"
      ...
      " . set-ant [<antfile>]    -- copies <antfile> into the annotation "
      "chunk\n"
      " . set-meta [<metafile>]  -- copies <metafile> into the metadata "
      "annotation tag\n"
      " . set-txt [<txtfile>]    -- copies <txtfile> into the hidden text "
      "chunk\n"
      " . set-xmp [<xmpfile>]    -- copies <xmpfile> into the xmp metadata "
      "annotation tag\n"
      ...
      "   remove-meta            -- removes metadatas without changing other "
      "annotations\n"
      "   remove-txt             -- removes hidden text\n"
      " _ remove-outline         -- removes outline (bookmarks)\n"
      " . remove-xmp             -- removes xmp metadata from annotation "
      "chunk\n"
      ...
      " . save-page-with <name>  -- saves selected page/file, inserting all "
      "included files\n"
...
void command_help(ParsingByteStream &) { command_help(); }
...
void usage() {
  DjVuPrintErrorUTF8(
      ...
      "  -u               -- produces utf8 instead of escaping non ascii "
      "chars\n"
...
void execute() {
  ...
  while (!!(token = cmd.get_token(true))) {
    ...
    G_CATCH(ex) {
      vprint("Error (%s): %s", (const char *)ToNative(token), ex.get_cause());
      if (!verbose)
        G_RETHROW;
    }
    G_ENDCATCH;
