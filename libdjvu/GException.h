...
#ifndef LIBDJVU_GEXCEPTION_H_
#define LIBDJVU_GEXCEPTION_H_
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
/** @name GException.h
    ...
    There are four macros for handling exceptions.  Macros #G_TRY#, #G_CATCH#
   and #G_ENDCATCH# are used to define an exception catching block.  Exceptions
   can be thrown at all times using macro #G_THROW(cause)#. An exception can be
...
/** Exception class.
    The library always uses macros #G_TRY#, #G_THROW#, #G_CATCH# and
   #G_ENDCATCH# for throwing and catching exceptions (see \Ref{GException.h}).
   These macros only deal with exceptions of type #GException#. */
...
class DJVUAPI GException {
 public:
  ...
  /** Constructs a GException.  This constructor is usually called by macro
      ...
      convention, string #ByteStream::EndOfFile# is used when reaching an
     unexpected end-of-file condition and string #DataPool::Stop# is used when
     the user interrupts the execution. The remaining arguments are usually
     provided by the predefined macros #__FILE__#, #__LINE__#, and (G++ and EGCS
     only)
      #__PRETTY_FUNCTION__#.  */
