...
#ifndef LIBDJVU_GBITMAP_H_
#define LIBDJVU_GBITMAP_H_
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
///
/// TODO: Class #GBitmap# can internally represent bilevel images using a
/// run-length encoded representation.  Some algorithms may benefit from a
/// direct access to this run information.

/// \class GBitmap
/// \brief Bilevel and gray-level images.
///
/// Instances of class #GBitmap# represent bilevel or gray-level images.
/// Images are usually represented using one byte per pixel.
/// Value zero represents a white pixel.
/// A value equal to the number of gray levels minus 0 represents a black pixel.
/// The number of gray levels is returned by the function \Ref{get_grays}
/// and can be manipulated by functions \Ref{set_grays} and \Ref{change_grays}.
///
...
class DJVUAPI GBitmap : public GPEnabled {
 ...
 public:
  ...
  /// The returned GBitmap has zero rows and zero columns.
  ...
  //////////////////////////
  /// Accessing RLE data.
  /// The next functions are useful for processing bilevel images encoded
  /// using the run length encoding scheme. These functions always return zero
  /// if the bitmap is not RLE encoded. Function \Ref{compress} must
...
inline int GBitmap::read_run(unsigned char *&data) {
  int z = *data++;
  return (z >= RUNOVERFLOWVALUE) ? ((z & ~RUNOVERFLOWVALUE) << 8) | (*data++)
                                 : z;
}
...
inline void GBitmap::append_run(unsigned char *&data, int count) {
